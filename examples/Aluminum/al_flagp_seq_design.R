...
model.em = flagp(X.sim = X_init$lhs, Y.sim = t(Y_init), y.ind.sim = matrix(time_all),
                 ...
                 nug.est = F, ls.parallel = T, rsvd = F, ls.nugget = 1e-8)
...
for(i in 1:n_points_to_add){
  ...
  if(m%%50==0 | i==n_points_to_add){
    if(method %in% c('maximin','maximin-scaled')){
      save.image(file = paste0('examples/Aluminum/seq_design/',method,'_refit_',refitevery,'_test_set_no_boot_results.RData'))
    } else{
      save.image(file = paste0('examples/Aluminum/seq_design/',method,'_',n.pc.sd,'_PC_refit_',refitevery,'_test_set_no_boot_results.RData'))
    }
  }
}
